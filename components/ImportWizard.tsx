...
const ImportWizard: React.FC<ImportWizardProps> = ({ onComplete, onCancel, currentData }) => {
  ...
  return (
    <div className="bg-white/90 backdrop-blur-xl rounded-3xl border border-white/50 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
      ...
      <div className="p-8 min-h-[400px]">
        {step === 1 && (
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
            <div className="border-2 border-dashed border-gray-300/60 bg-white/40 rounded-2xl p-10 text-center relative group hover:border-blue-400 transition-all backdrop-blur-sm">
            ...
            {files.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {files.map((f, i) => (
                  <div key={i} className="bg-white/50 p-3 rounded-xl flex items-center justify-between border border-white/50 backdrop-blur-sm">
                    <div className="flex items-center gap-2">
                      <FileText className="w-4 h-4 text-gray-400" />
                      <span className="text-xs font-bold text-gray-700 truncate max-w-[150px]">{f.name}</span>
                    </div>
                    <button onClick={() => setFiles(prev => prev.filter((_, idx) => idx !== i))} className="p-1 hover:bg-white/80 rounded-lg text-gray-400">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <button onClick={() => setMode('complete')} className={`p-4 rounded-xl border text-left transition-all backdrop-blur-sm ${mode === 'complete' ? 'border-blue-600 bg-blue-50/50' : 'border-gray-100 bg-white/40 hover:border-blue-200'}`}>
                    ...
                    <button onClick={() => setMode('incomplete')} className={`p-4 rounded-xl border text-left transition-all backdrop-blur-sm ${mode === 'incomplete' ? 'border-blue-600 bg-blue-50/50' : 'border-gray-100 bg-white/40 hover:border-blue-200'}`}>
                      <div className="flex items-center justify-between mb-1">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <button onClick={() => setStructure('multi-file-single')} className={`p-4 rounded-xl border text-left transition-all backdrop-blur-sm ${structure === 'multi-file-single' ? 'border-orange-600 bg-orange-50/50' : 'border-gray-100 bg-white/40 hover:border-orange-200'}`}>
                      <div className="flex items-center justify-between mb-1">
                    ...
                    <button onClick={() => setStructure('single-file-multi')} className={`p-4 rounded-xl border text-left transition-all backdrop-blur-sm ${structure === 'single-file-multi' ? 'border-orange-600 bg-orange-50/50' : 'border-gray-100 bg-white/40 hover:border-orange-200'}`}>
                      <div className="flex items-center justify-between mb-1">
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <button onClick={() => setRankSource('recalculate')} className={`p-4 rounded-xl border text-left transition-all backdrop-blur-sm ${rankSource === 'recalculate' ? 'border-indigo-600 bg-indigo-50/50' : 'border-gray-100 bg-white/40 hover:border-indigo-200'}`}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setRankSource('imported')} className={`p-4 rounded-xl border text-left transition-all backdrop-blur-sm ${rankSource === 'imported' ? 'border-indigo-600 bg-indigo-50/50' : 'border-gray-100 bg-white/40 hover:border-indigo-200'}`}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                ...
                {structure === 'single-file-multi' && (
                  <div className="space-y-2 p-5 bg-white/40 rounded-2xl border border-white/50 animate-in fade-in slide-in-from-top-2 backdrop-blur-sm">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t('wizard.define_sequence')}</label>
                    <input type="text" value={periodNamesInput} onChange={(e) => setPeriodNamesInput(e.target.value)} placeholder="e.g. Sep Monthly, Mid-term, Dec Monthly..." className="w-full bg-white/60 border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <select value={mapping.name} onChange={(e) => setMapping({...mapping, name: e.target.value})} className="w-full bg-white/50 border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-blue-500">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <select value={mapping.class} onChange={(e) => setMapping({...mapping, class: e.target.value})} className="w-full bg-white/50 border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-blue-500">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <select disabled={rankSource === 'recalculate'} value={mapping.totalRank} onChange={(e) => setMapping({...mapping, totalRank: e.target.value})} className="w-full bg-white/50 border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <select disabled={rankSource === 'recalculate'} value={mapping.status} onChange={(e) => setMapping({...mapping, status: e.target.value})} className="w-full bg-white/50 border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50">
                      <option value="">{t('wizard.not_specified')}</option>
                      {headers.map(h => <option key={h} value={h}>{h}</option>)}
                    </select>
                  </div>
            ...
            <div className="space-y-4 pt-6">
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                {availableSubjectColumns.map(h => (
                  <div key={h} className={`bg-white/40 p-4 rounded-xl flex items-center justify-between border transition-all backdrop-blur-sm ${mapping.subjects[h] ? 'border-blue-200 bg-blue-50/40' : 'border-white/50 hover:border-blue-300'}`}>
                    ...
                    <div className="flex items-center gap-4">
                      {rankSource === 'imported' && mapping.subjects[h] && (
                        <div className="flex items-center gap-2 bg-white/70 px-3 py-1.5 rounded-lg border border-gray-100 shadow-sm animate-in fade-in slide-in-from-right-1">
      ...
      <div className="bg-white/40 p-8 flex justify-between items-center border-t border-white/50 backdrop-blur-sm">
        ...
        <div className="flex gap-4">
          {step > 1 && (
            <button onClick={() => setStep(step - 1)} className="px-8 py-3 rounded-xl border border-gray-200 font-bold text-gray-600 hover:bg-white/50 transition-all flex items-center gap-2">
