...
const StudentDetailView: React.FC<StudentDetailViewProps> = ({ 
  ...
  const getSubjectCellStyle = useCallback((period: string, subject: string, studentName: string) => {
    ...
    const styles: Record<string, string> = {
      'king': 'bg-purple-100/50 text-purple-900 border-purple-200 font-bold border-b-2',
      'elite': 'bg-green-100/50 text-green-900 border-green-200 font-bold border-b-2',
      'high': 'bg-blue-100/50 text-blue-900 border-blue-200 font-bold border-b-2',
      'standard': 'text-gray-600',
      'pass': 'bg-yellow-100/50 text-yellow-900 border-yellow-200 font-bold border-b-2',
      'fail': 'bg-red-50/50 text-red-700 border-red-100 font-bold border-b-2'
    };
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-3 bg-white/50 border border-white/50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all backdrop-blur-sm"
            />
          </div>
          <div className="flex items-center gap-2">
            <Filter className="w-4 h-4 text-gray-400" />
            <select 
              className="bg-white/50 border border-white/50 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-500 backdrop-blur-sm cursor-pointer"
          ...
          {selectableStudents.map(s => (
            <button
              ...
              className={`text-left p-3 rounded-xl border transition-all text-xs flex flex-col gap-1 ${
                selectedStudentId === s.id ? 'bg-blue-600 border-blue-600 text-white shadow-md shadow-blue-500/30' : 'bg-white/40 border-white/30 hover:bg-white/80 hover:border-blue-300 text-slate-700'
              }`}
            >
              <span className="font-bold truncate">{s.name}</span>
              <span className={`text-[10px] opacity-70 ${selectedStudentId === s.id ? 'text-blue-100' : 'text-slate-400'}`}>{s.class}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 space-y-6">
              <div className="bg-gradient-to-br from-indigo-600/90 to-blue-700/90 backdrop-blur-md p-8 rounded-2xl shadow-xl text-white border border-white/10">
              ...
              {streakInfo && streakInfo.count > 0 && (
                <div className={`p-6 rounded-2xl border shadow-sm transition-all animate-in slide-in-from-left-4 backdrop-blur-sm ${streakInfo.type === 'improvement' ? 'bg-emerald-50/60 border-emerald-100' : 'bg-rose-50/60 border-rose-100'}`}>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-end"><span className="text-xs text-slate-500">总位次变动</span><span className={`text-2xl font-black ${streakInfo.type === 'improvement' ? 'text-emerald-600' : 'text-rose-600'}`}>{streakInfo.totalChange > 0 ? `+${streakInfo.totalChange}` : streakInfo.totalChange}</span></div>
                    <div className="flex flex-wrap gap-2">{streakInfo.steps.map((step, idx) => (<div key={idx} className={`px-2 py-1 rounded-lg text-[10px] font-bold flex items-center gap-1 ${streakInfo.type === 'improvement' ? 'bg-emerald-200/50 text-emerald-800' : 'bg-rose-200/50 text-rose-800'}`}>{streakInfo.type === 'improvement' ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}{step > 0 ? `+${step}` : step}</div>))}</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="lg:col-span-2 space-y-8">
              <ChartContainer title="📉 School Ranking Trend (Upwards is Better)">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" /><XAxis dataKey="period" stroke="#94a3b8" fontSize={12} /><YAxis reversed domain={['auto', 'auto']} stroke="#94a3b8" fontSize={12} />
                    <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', background: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(10px)', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }} formatter={(value: number) => [`第 ${value} 名`, 'School Rank']} />
              ...
              <ChartContainer title="📈 Performance vs Grade Average Curve">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" /><XAxis dataKey="period" stroke="#94a3b8" fontSize={12} /><YAxis domain={['auto', 'auto']} stroke="#94a3b8" fontSize={12} />
                    <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', background: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(10px)', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }} formatter={(value: number) => [value.toFixed(1), '']} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <ChartContainer title={`📊 Subject Competency Radar (${selectedPeriod})`}>
              <div className="absolute top-6 right-8 flex gap-2">
                <button 
                  ...
                  className={`px-3 py-1 rounded-lg text-[10px] font-bold transition-all ${radarBaseline === 'class' ? 'bg-blue-600 text-white' : 'bg-white border border-slate-200 text-slate-400'}`}
                >
                  班级平均
                </button>
                <button 
                  onClick={() => setRadarBaseline('grade')}
                  className={`px-3 py-1 rounded-lg text-[10px] font-bold transition-all ${radarBaseline === 'grade' ? 'bg-blue-600 text-white' : 'bg-white border border-slate-200 text-slate-400'}`}
                >
                  年级平均
                </button>
              </div>
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
                  ...
                  <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', background: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(10px)', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }} />
            ...
            <ChartContainer title={`📈 ${selectedSubjectForTrend} Historical Rank Trend`}>
              <div className="absolute top-6 right-8">
                <select 
                  className="bg-white/50 border border-slate-200 rounded-xl px-3 py-1 text-[10px] font-bold outline-none focus:ring-2 focus:ring-blue-500 backdrop-blur-sm"
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={subjectTrendData}>
                  ...
                  <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', background: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(10px)', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }} formatter={(value: number) => [`第 ${value} 名`, 'Subject Rank']} />
                  <Legend verticalAlign="bottom" height={36} />
